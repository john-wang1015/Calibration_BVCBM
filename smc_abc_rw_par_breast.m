function [part_vals, part_sim, part_s, sims,dist_t,p_acc_t,dist_history,sims_history] = smc_abc_rw_par_breast(y,sim_params,...
                                                 dist_func,...
                                                 prior,N,dist_final,...
...
parfor i = 1:N
    ...
    % simulate model
    part_sim(i,:) = simulatorSecondBip(part_vals(i,:), sim_params, total_time);
...
while (dist_max > dist_final)
    ...
    parfor i = (num_keep+1):N
        for r = 1:mcmc_trials
            ...
            while findBreastLimit(part_vals_prop,prior) == 0
                part_vals_prop = mvnrnd(part_vals(i,:),cov_matrix);
            end
...
%             prop = prior.trans_finv(part_vals_prop);
            ...
            part_sim_prop = simulatorSecondBip(part_vals_prop, sim_params, total_time);
